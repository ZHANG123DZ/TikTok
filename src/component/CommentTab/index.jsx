...
import { createComment, getCommentsByPostId } from "../../services/Comments/Comments.service";
...
function CommentTab({post_id}) {
  ...
  return <aside className={styles.AsideOneColumnSidebar}>
<div className={styles.DivFeedNavigationContainer}>
    {/* Bổ sung sau */}
</div>
<div className={styles.DivCommentSidebarTransitionWrapper}>
    <section className={styles["BaseCommentContainerStyles-SectionCommentSidebarContainer"]}>
    <div className={styles.DivCommentHeader}>
        <div className={styles.DivCommentHeaderTextWrapper}>
          <h4 className={clsx("TUXText", "TUXText--weight-bold", styles.StyledTUXText)} style={{fontSize: '15px', color: 'inherit'}}>Comments</h4>
          <h4 className={clsx("TUXText", "TUXText--weight-medium", styles.StyledTUXText)} style={{fontSize: '15px', color: 'inherit'}}>({Array.isArray(commentsList.data) &&commentsList.data.length})</h4>
        </div>
    </div>
    <div className={styles.DivCommentMain}>
      <div className={styles.DivCommentListContainer}>
        {Array.isArray(commentsList.data) &&
            ...
            comment && 
            <div className={styles.DivCommentObjectWrapper} key={comment.id}>
              ...
              <div className={styles.DivLayoutContainer}>
                <div className={styles.DivInputAreaContainer}>
                  <div className={styles.DivInputEditorContainer}>
                    <div className={styles.DivInputAreaContainer} style={{ minWidth: '100px' }}>
                      <div className="DraftEditor-root">
                        <div className="public-DraftEditorPlaceholder-root">
                          <input type="text" placeholder="Add comment..." className={styles.InputContainer} value={content} onChange={(event) => setContent(event.currentTarget.value)}/>
                        </div>
                      </div>   
                  ...
                  <div className="TUXTooltip-reference">
                    <Button icon={<FontAwesomeIcon icon={faAt}/>} mentionSuggest secondary isDefault size="medium"/>
                  </div>    
                  <div className="TUXTooltip-reference">
                    <Button icon={<FontAwesomeIcon icon={faFaceLaugh}/>} emoji secondary isDefault size="medium"/>
                  </div>    
                  </div>
              </div>
              <button className={styles.PostButton} role="button">Đăng</button>
            </div>
          </form>
        </div>
      </div>
